...
- `driftguard-demo` с React UI, REST API, Swagger UI, synthetic scenarios, Kafka integration demo, configuration view и quality metrics.
...
| `driftguard-demo`                | Demo-приложение с UI, REST API, OpenAPI и Kafka integration demo.                       |
...
Dashboard показывает overview, synthetic scenarios, Kafka streams, события detector-ов, текущую конфигурацию и quality metrics.
...
Раздел `Configuration` показывает Kafka topics, Kafka Streams application id, playback interval, detector definitions, warning/critical thresholds, p-value thresholds, emission policy и агрегированную оценку чувствительности: `Aggressive`, `Balanced` или `Conservative`.

Для локальной разработки frontend отдельно от Spring Boot:
