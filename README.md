...
Проект задуман не как один монолитный сервис, а как набор библиотек и adapter-слоёв. Core-логика не зависит от Spring, Kafka, Jackson, Micrometer или конкретного транспорта. Это позволяет использовать DriftGuard в Kafka Streams, Spring Boot приложениях, batch/replay экспериментах и тестовых сценариях.

## Что Уже Есть

- чистый `driftguard-core` с domain model, detector API, engine и state store abstraction;
...
- `driftguard-demo` с REST API, synthetic degradation scenario и quality metrics.
...
| `driftguard-demo` | Demo-приложение с REST API. |
...
| `adwin` | Adaptive-window mean shift detector для потоковой обработки. Сейчас реализован MVP ADWIN-style на sliding window и Hoeffding bound. |
...
- Имена topic-ов, detector-ы, окна и пороги должны приходить из конфигурации.
...
Kafka-модуль уже содержит SerDes и topology builder. Текущая topology вызывает `DriftDetectorEngine` напрямую. Для production-сценариев следующий шаг - durable state через Kafka Streams state store и сериализация detector state. Это намеренно оставлено отдельным слоем, чтобы не смешивать начальную core-модель с transport-specific persistence.
